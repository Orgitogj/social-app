...
const EditProfile = () => {
  ...
  const onSubmit = async () => {
    ...
    let updatedUser: UserProfileForm = { ...user }

    if (updatedUser.image && typeof updatedUser.image === 'object') {
      let imageRes = await uploadFile('profiles', updatedUser.image.uri, true)
      if (imageRes.success && imageRes.data) {
        updatedUser.image = imageRes.data
      } else {
        Alert.alert('Profile', imageRes.msg || 'Could not upload image')
        setLoading(false)
        return
      }
    }
