...
const Home = () => {
    ...
    if (payload.eventType === 'DELETE' && payload?.old?.postId) {
      ...
      setPosts(prevPosts => {
        const updated = prevPosts.map(post => {
          ...
          return {
            ...post,
            comments: [{ count: Math.max(currentCount - 1, 0) }]
          };
        });
        postsRef.current = updated;
        return updated;
  ...
  <HugeiconsIcon icon={Search01Icon} size={hp(3.2)} strokeWidth={2} color={theme.colors.text} />
</Pressable>
