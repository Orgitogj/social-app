...
The Home Stories bar reads one grouped summary per author from `get_story_tray` (own stories first, then authors with unviewed stories, then recent activity) without downloading media. `story_views` holds one row per viewer and story, written only by the idempotent `mark_story_viewed` RPC and readable only by that viewer. A story counts as viewed after one second of visible playback (or when a shorter video ends); the ring updates through targeted React Query cache writes under `storyKeys`. Story creation prepares images (JPEG, at most 1920 px) and validates videos (MP4, MOV or WebM, 1 to 60 seconds, up to 50 MB) before uploading with progress to the story folder, then publishes with a stable story id so retries never duplicate records or objects; rejected or discarded drafts remove their uploads. The fullscreen viewer plays images for five seconds and videos for their real duration, pauses on hold, in the background, or when unfocused, and skips stories that expire, are deleted, or become inaccessible.

Story authors read their own viewer list through `get_story_viewers`, a cursor-paginated query protected by a row policy that only matches the author of the story; other viewers still see only their own view rows. Replies and quick reactions are ordinary one-to-one messages: `send_story_interaction` checks that the story is visible and unexpired, reuses or starts the conversation under the existing messaging rules, and inserts a `story_reply` or `story_reaction` message carrying a small durable snapshot of the story (id, author, media type, creation time). `message_document` adds a live preview only while the story is still visible to the reader, so expired or revoked stories render as a placeholder in chat without keeping media reachable. Story captions reuse the shared `@mention` parser; mentions are recorded in `story_mentions` and notify only users who can already see the story, through the existing notifications, preferences, dedupe keys and push queue. A mention never grants access.
